...
# Write to a CSV file
results_df.to_csv('predictions_vs_actuals_LSTM.csv', index=False)
'''
...
def evaluate_performance(top_features, X_train, X_test, y_train, y_test):
    model = Sequential([
        LSTM(50, activation='relu', input_shape=(1, len(top_features))),
        Dense(16, activation='relu'),
        Dense(1, activation='linear')
    ])
    model.compile(optimizer='adam', loss='mse')
...
print(f"Model performance (MSE) with top features - baseline: {mse}, LRP: {mse_lrp_normal},LRP_ENHANCED: {mse_lrp_enhanced}, SHAP: {mse_shap}, LIME: {mse_lime}, COMBINED: {mse_combined},COMBINED_ENHANCED: {mse_combined_enhanced}")
...
filename = f"LSTM-Hybrid-heatmap-{num_rows}.jpg"  # Create the filename with the number of rows
