...
# Write to a CSV file
results_df.to_csv('predictions_vs_actuals_RNN.csv', index=False)
'''
...
def evaluate_performance(top_features, X_train, X_test, y_train, y_test):
    model = Sequential([
        SimpleRNN(50, activation='relu', input_shape=(1, len(top_features))),
        Dense(16, activation='relu'),
        Dense(1, activation='linear')
    ])
    model.compile(optimizer='adam', loss='mse')
...
filename = f"RNN-Hybrid-heatmap-{num_rows}.jpg"  # Create the filename with the number of rows
